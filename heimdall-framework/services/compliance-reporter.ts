 ...
 * 
 * Generates compliance reports and calculates scores
 */

import { TestResult, ComplianceScore, ReportingConfig } from '../core/types';
...
export class ComplianceReporter {
   ...
   */
  async generateReport(results: TestResult[]): Promise<any> {
  ...
  calculateScores(results: TestResult[]): {
    ...
    const byApplication: Record<string, { passed: number; total: number }> = {};
    const byTeam: Record<string, { passed: number; total: number }> = {};
    ...
    for (const result of results) {
      ...
      // By application (would need to extract from result details)
      ...
      // By team (would need to extract from result details)
    ...
    return {
      ...
      byApplication: Object.fromEntries(
        Object.entries(byApplication).map(([key, value]) => [
          key,
          value.total > 0 ? Math.round((value.passed / value.total) * 100 * 100) / 100 : 100,
        ])
      ),
      byTeam: Object.fromEntries(
        Object.entries(byTeam).map(([key, value]) => [
          key,
          value.total > 0 ? Math.round((value.passed / value.total) * 100 * 100) / 100 : 100,
        ])
      ),
   ...
   * Generate summary
   */
  private generateSummary(results: TestResult[], scores: any): any {
  ...
  /**
   * Extract application from test result
   */
  private extractApplication(result: TestResult): string | null {
    // Would extract from result details or test name
    return result.details?.application || null;
  }

  /**
   * Extract team from test result
   */
  ...
  private async writeReport(report: any): Promise<void> {
    ...
    const filePath = path.join(
      ...
      `compliance-report-${Date.now()}.${extension}`
    );
...
<body>
  ...
  private generateJunitReport(report: any): string {
    const testsuites = report.results.map((r: TestResult) => {
