...
export class DLPController {
  ...
  async testExfiltration(@Body(ValidationPipe) dto: { configId?: string; user?: any; dataOperation?: any }) {
    this.logger.log(dto.configId
      ? `Testing exfiltration with config: ${dto.configId}`
  ...
  async validateAPIResponse(
    @Body(ValidationPipe) dto: { configId?: string; apiResponse: any; allowedFields?: string[]; piiFields?: string[] },
  ) {
    this.logger.log(dto.configId
      ? `Validating API response with config: ${dto.configId}`
  ...
  async testQueryValidation(
    @Body(ValidationPipe) dto: { configId?: string; query: any; user: any; expectedFields?: string[] },
  ) {
    this.logger.log(dto.configId
      ? `Testing query validation with config: ${dto.configId}`
  ...
  @HttpCode(HttpStatus.OK)
  async testBulkExport(@Body(ValidationPipe) dto: { configId?: string; user?: any; exportRequest?: any }) {
    this.logger.log(dto.configId
      ? `Testing bulk export with config: ${dto.configId}`
