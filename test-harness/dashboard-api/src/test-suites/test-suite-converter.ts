...
export function parseTypeScriptTestSuite(tsContent: string, filePath: string): ParsedTestSuite | null {
  try {
    ...
    if (constExportMatch) {
      ...
      if (configStr.includes('includeContractTests') && configStr.match(/includeContractTests\s*:\s*true/)) {
...
function extractTestSuiteProperties(tsContent: string): TestSuite | null {
  try {
    ...
    suite.includeContractTests = /includeContractTests\s*:\s*true/.test(tsContent);
