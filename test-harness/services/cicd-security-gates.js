...
exports.CICDSecurityGates = void 0;
const iac_scanner_1 = require("./iac-scanner");
const container_security_scanner_1 = require("./container-security-scanner");
const k8s_rbac_validator_1 = require("./k8s-rbac-validator");
class CICDSecurityGates {
    constructor(config, containerScanner, k8sValidator, policyValidator) {
        if (config && 'scanFiles' in config) {
            this.config = {};
            this.iacScanner = config;
            this.containerScanner = containerScanner || new container_security_scanner_1.ContainerSecurityScanner();
            this.k8sValidator = k8sValidator || new k8s_rbac_validator_1.K8sRBACValidator();
            this.policyValidator = policyValidator;
        }
        else {
            this.config = config || {};
            this.iacScanner = this.config.iacScanner || new iac_scanner_1.IACScanner();
            this.containerScanner = this.config.containerScanner || new container_security_scanner_1.ContainerSecurityScanner();
            this.k8sValidator = this.config.k8sValidator || new k8s_rbac_validator_1.K8sRBACValidator();
            this.policyValidator = this.config.policyValidator;
        }
    }
    async validatePreMergePolicies(pr, policies) {
        const gates = [];
        const findings = [];
    ...
    async checkSecurityGates(pr, config) {
        const gates = [];
        const findings = [];
        ...
        if (config.scanIAC) {
            ...
            const iacFiles = pr.files.filter(f => iacPatterns.some(pattern => f.endsWith(pattern) || f.includes(pattern)));
        ...
        if (config.scanContainers) {
            ...
            const dockerFiles = pr.files.filter(f => containerPatterns.some(pattern => f.includes(pattern)));
        ...
        if (config.validateK8sRBAC) {
            ...
            const k8sFiles = pr.files.filter(f => (f.endsWith('.yaml') || f.endsWith('.yml')) &&
        ...
        if (config.failOnThreshold && !passed) {
            ...
            const hasThresholdViolation = findings.some(f => this.severityLevel(f.severity) >= this.severityLevel(thresholdSeverity));
...
exports.CICDSecurityGates = CICDSecurityGates;
//# sourceMappingURL=cicd-security-gates.js.map