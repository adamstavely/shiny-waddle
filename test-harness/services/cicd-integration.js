...
class CICDIntegration {
    async runPreCommitHook(stagedFiles, findings, config) {
        ...
        const blockingFindings = relevantFindings.filter(f => {
            if (config.allowedFindings?.includes(f.id))
                return false;
    ...
    async runPRSecurityCheck(baseFindings, headFindings, config) {
    ...
    async runBuildSecurityGate(findings, config) {
    ...
    async runDeploymentSecurityGate(findings, environment, config) {
        const riskScore = this.calculateRiskScore(findings);
