          ...
          <div class="modal-body">
            <form @submit.prevent="save" class="battery-form">
              ...
              <div class="form-section">
                ...
                <div v-if="hasDuplicateDomains" class="validation-warning">
...
import type { TestBattery, BatteryExecutionConfig } from '../../heimdall-framework/core/types';
...
const selectedHarnessTypes = computed(() => {
  ...
  form.value.harnessIds.forEach(harnessId => {
    ...
    if (harness && harness.domain) {
      domains.add(harness.domain);
    }
  });
  return domains;
});

const hasDuplicateDomains = computed(() => {
...
const save = async () => {
  if (hasDuplicateDomains.value) {
    alert('All harnesses in a battery must have different domains. Please remove duplicate domains.');
    saving.value = false;
    return;
  }
