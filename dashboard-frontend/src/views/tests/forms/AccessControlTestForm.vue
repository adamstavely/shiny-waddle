<template>
  <div class="access-control-test-form">
    ...
    <div class="form-group">
      <label>Policy to Test *</label>
      <p class="form-help">Select a policy to test against (1:1 relationship)</p>
      <div class="policy-selector">
        ...
        <div v-else class="policies-list">
          <div
            ...
            :key="policy.id"
            class="policy-option"
            :class="{ selected: form.policyId === policy.id }"
            @click="selectPolicy(policy.id)"
          >
            <input
              type="radio"
              :checked="form.policyId === policy.id"
              @change="selectPolicy(policy.id)"
            />
    ...
    <div class="form-group">
      ...
      <Dropdown
        v-model="form.inputs.subject.role"
        :options="roleOptions"
      ...
      <div class="resource-config">
        <div class="form-grid">
          <div class="form-group">
            ...
            <input v-model="form.inputs.resource.id" type="text" required class="form-input" />
          </div>
          <div class="form-group">
            <label>Resource Type</label>
            <input v-model="form.inputs.resource.type" type="text" required class="form-input" />
          </div>
          <div class="form-group">
            <label>Sensitivity</label>
            <Dropdown
              v-model="form.inputs.resource.sensitivity"
      ...
      <div class="context-config">
        <div class="form-grid">
          <div class="form-group">
            ...
            <input v-model="form.inputs.context.ipAddress" type="text" class="form-input" />
          </div>
          <div class="form-group">
            <label>Time of Day</label>
            <input v-model="form.inputs.context.timeOfDay" type="text" class="form-input" />
          </div>
          <div class="form-group">
            <label>Location</label>
            <input v-model="form.inputs.context.location" type="text" class="form-input" />
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <Dropdown
        v-model="expectedDecisionValue"
        :options="expectedDecisionOptions"
        placeholder="Select expected decision..."
        required
        class="form-input"
        @update:model-value="updateExpectedDecision"
      />
    </div>
  </div>
</template>
...
const expectedDecisionValue = computed(() => {
  return props.form.expected?.allowed ? 'allow' : 'deny';
});

const selectPolicy = (policyId: string) => {
  props.form.policyId = policyId;
};

const updateExpectedDecision = (value: string) => {
  if (!props.form.expected) {
    props.form.expected = { allowed: false };
  }
  props.form.expected.allowed = value === 'allow';
};
