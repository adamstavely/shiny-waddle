  ...
  <div class="policies-page">
    ...
    <div class="page-header">
      <div class="header-content">
        <div>
          <h1 class="page-title">ServiceNow Data Protection Baselines</h1>
          <p class="page-description">Manage ServiceNow data protection baselines for HIPAA compliance</p>
        </div>
    ...
    <div v-else-if="baselines.length === 0" class="empty-state">
      ...
      <p>Create your first ServiceNow data protection baseline to get started</p>
          ...
          <div class="modal-content" @click.stop>
            <div class="modal-header">
              <div class="modal-title-group">
                ...
                <h2>Create ServiceNow Data Protection Baseline</h2>
              </div>
            ...
            <div class="modal-body">
              <form @submit.prevent="saveBaseline" class="form">
                ...
                <div class="form-section">
                  ...
                  <div v-if="baselineForm.config.encryption.fieldEncryption.enabled" class="form-group">
                    ...
                    <select v-model="baselineForm.config.encryption.fieldEncryption.keyManagement">
                      <option value="ServiceNow Managed">ServiceNow Managed</option>
                      <option value="Customer Managed">Customer Managed</option>
                    </select>
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.encryption.databaseEncryption.enabled" />
                      <span>Enable Database Encryption (TDE)</span>
                    </label>
                    <p class="form-hint">Transparent Data Encryption for database-level protection</p>
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.encryption.sslTls.enabled" />
                      <span>Enable SSL/TLS Encryption</span>
                    </label>
                    <p class="form-hint">Required for HIPAA Security Rule compliance (encryption in transit)</p>
                  </div>
                </div>
                ...
                <div class="form-section">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.requireACLs" />
                      <span>Require ACLs for Data Access</span>
                    </label>
                    <p class="form-hint">Enforce Access Control Lists for PHI tables</p>
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.requireRBAC" />
                      <span>Require Role-Based Access Control</span>
                    </label>
                    <p class="form-hint">Enforce RBAC for all data access</p>
                  </div>
                </div>
                ...
                <div class="form-section">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.auditLogging.tableAuditEnabled" />
                      <span>Enable Table Audit</span>
                    </label>
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.auditLogging.fieldAuditEnabled" />
                      <span>Enable Field Audit</span>
                    </label>
                    <p class="form-hint">Track field-level changes (required for HIPAA)</p>
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                  ...
                  <div v-if="baselineForm.config.auditLogging.hipaaCompliance.enabled" class="form-group">
                    <label>Audit Retention Period (days)</label>
                    <input v-model.number="baselineForm.config.auditLogging.hipaaCompliance.auditRetention" type="number" min="2190" placeholder="2190 (6 years minimum)" />
                    <p class="form-hint">HIPAA requires minimum 6 years (2190 days) retention</p>
                  </div>
                </div>
                ...
                <div class="form-section">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.dataRetention.enableArchiving" />
                      <span>Enable Data Archiving</span>
                    </label>
                    <p class="form-hint">Archive data before deletion for compliance</p>
                  </div>
                  <div class="form-group">
                ...
                <div class="form-section">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.apiSecurity.restApi.authenticationRequired" />
                      <span>Require REST API Authentication</span>
                    </label>
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.apiSecurity.restApi.oauthEnabled" />
                      <span>Enable OAuth for REST API</span>
                    </label>
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.apiSecurity.restApi.rateLimiting.enabled" />
                ...
                <div class="form-section">
                  ...
                  <div v-if="baselineForm.config.externalDataSharing.dataExport.allowed" class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.externalDataSharing.dataExport.hipaaDataRequiresApproval" />
                      <span>Require Approval for HIPAA Data Export</span>
                    </label>
                    <p class="form-hint">Additional approval required for PHI data exports</p>
                  </div>
                </div>
...
const baselineForm = ref({
  ...
  config: {
    encryption: {
      fieldEncryption: {
        enabled: false,
        keyManagement: 'ServiceNow Managed' as 'ServiceNow Managed' | 'Customer Managed'
      },
      databaseEncryption: {
        enabled: false,
        tdeEnabled: false
    ...
    externalDataSharing: {
      dataExport: {
        ...
        hipaaDataRequiresApproval: false
      }
    }
  }
});
...
const loadBaselines = async () => {
  ...
  try {
    const response = await axios.get('/api/v1/servicenow/baselines');
...
const saveBaseline = async () => {
  try {
    ...
    await axios.post('/api/v1/servicenow/baselines', payload);
    await loadBaselines();
    closeBaselineModal();
  } catch (err: any) {
...
const deleteBaseline = async (id: string) => {
  if (confirm('Are you sure you want to delete this baseline?')) {
    try {
      await axios.delete(`/api/v1/servicenow/baselines/${id}`);
      await loadBaselines();
    } catch (err: any) {
