  ...
  <div class="policies-page">
    ...
    <div class="page-header">
      <div class="header-content">
        <div>
          <h1 class="page-title">IDP / Kubernetes Data Protection Baselines</h1>
          <p class="page-description">Manage IDP and Kubernetes data protection baselines for HIPAA compliance</p>
        </div>
    ...
    <div v-else-if="baselines.length === 0" class="empty-state">
      <Container class="empty-icon" />
      <h3>No baselines defined</h3>
      <p>Create your first IDP/Kubernetes data protection baseline to get started</p>
          ...
          <div class="modal-content" @click.stop>
            <div class="modal-header">
              <div class="modal-title-group">
                <Container class="modal-title-icon" />
                <h2>Create IDP/Kubernetes Data Protection Baseline</h2>
              </div>
            ...
            <div class="modal-body">
              <form @submit.prevent="saveBaseline" class="form">
                ...
                <div class="form-section">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.secretsManagement.encryption.enabled" />
                      <span>Require Secrets Encryption</span>
                    </label>
                    <p class="form-hint">Encrypt all secrets at rest (required for PHI credentials)</p>
                  </div>
                  <div v-if="baselineForm.config.secretsManagement.encryption.enabled" class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.secretsManagement.encryption.keyRotation.enabled" />
                      <span>Enable Key Rotation</span>
                    </label>
                  </div>
                  <div v-if="baselineForm.config.secretsManagement.encryption.keyRotation.enabled" class="form-group">
                    <label>Rotation Interval (days)</label>
                    <input v-model.number="baselineForm.config.secretsManagement.encryption.keyRotation.interval" type="number" min="1" placeholder="90" />
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.secretsManagement.secretAccess.rbacRequired" />
                      <span>Require RBAC for Secret Access</span>
                    </label>
                  </div>
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.secretsManagement.secretAccess.auditLogging" />
                      <span>Enable Audit Logging for Secret Access</span>
                    </label>
                  </div>
                </div>
                ...
                <div class="form-section">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.encryptionAtRest.enabled" />
                      <span>Enable Encryption at Rest</span>
                    </label>
                    <p class="form-hint">Required for PHI data protection</p>
                  </div>
                  <div v-if="baselineForm.config.encryptionAtRest.enabled" class="form-group">
                    <label>Key Management Provider</label>
                    <select v-model="baselineForm.config.encryptionAtRest.provider">
                ...
                <div class="form-section">
                  ...
                  <div class="form-group">
                    <label class="checkbox-label">
                      <input type="checkbox" v-model="baselineForm.config.requireNetworkPolicies" />
                      <span>Require Network Policies for Data Isolation</span>
                    </label>
                    <p class="form-hint">Enforce network isolation for PHI workloads</p>
                  </div>
                </div>
                ...
                <div class="form-section">
                  ...
                  <div class="form-group">
                    ...
                    <select v-model="baselineForm.config.podSecurityStandards.enforce">
                      <option value="restricted">Restricted (Recommended for PHI)</option>
                      <option value="baseline">Baseline</option>
                      <option value="privileged">Privileged</option>
                    </select>
                    <p class="form-hint">Restricted mode recommended for data protection</p>
                  </div>
                </div>
                ...
                <div class="form-section">
                  ...
                  <div v-if="baselineForm.config.auditLogging.enabled" class="form-group">
                    ...
                    <select v-model="baselineForm.config.auditLogging.logLevel">
...
const baselineForm = ref({
  ...
  config: {
    ...
    encryptionAtRest: {
      enabled: false,
      provider: 'kubernetes' as 'kubernetes' | 'cloud-provider'
    },
    requireNetworkPolicies: true,
    podSecurityStandards: {
      enforce: 'restricted' as 'restricted' | 'baseline' | 'privileged'
    },
    auditLogging: {
      enabled: true,
      logLevel: 'RequestResponse' as 'None' | 'Metadata' | 'Request' | 'RequestResponse'
    }
  }
});
...
const loadBaselines = async () => {
  ...
  try {
    const response = await axios.get('/api/v1/idp-kubernetes/baselines');
...
const saveBaseline = async () => {
  try {
    ...
    await axios.post('/api/v1/idp-kubernetes/baselines', payload);
    await loadBaselines();
    closeBaselineModal();
  } catch (err: any) {
...
const deleteBaseline = async (id: string) => {
  if (confirm('Are you sure you want to delete this baseline?')) {
    try {
      await axios.delete(`/api/v1/idp-kubernetes/baselines/${id}`);
      await loadBaselines();
    } catch (err: any) {
